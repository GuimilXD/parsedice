...
static ParserItem parse_dice(StringSlice *p) {
  ...
  if (errno != 0 || !(parse_character(p, 'd'))) {
      // Return parser state to previous
    *p = tmp;
...
static ParserItem parse_operation(StringSlice *p) {
  for (size_t i = 0; i < sizeof(op_mappings) / sizeof(op_mappings[0]); i++) {
...
static inline ParserItem parse_const_int(StringSlice *p) {
  char *end_ptr;

  DiceInt res = strtol(p->start, &end_ptr, 10);
...
static ParserItem (*parsers[])(StringSlice *p) = {parse_dice, parse_const_int,
                                                  parse_operation};
